/* function that checks source attributes and saves it into session storage*/
...
function next_source_attribute() {
    if(document.getElementById("source_color").checked == false && document.getElementById("source_size").checked == false && document.getElementById("source_skip").checked == false){
        alert("Please select from options in order to continue!")
    }
    else{
    location.href='target_attributes_phos.html';}
}